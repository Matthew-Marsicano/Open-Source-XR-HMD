...
class KalmanFilter:
    def __init__(self, process_variance, measurement_variance, estimated_measurement_variance):
        ...
        self.estimated_measurement_variance = estimated_measurement_variance  # Estimated variance
        self.posteri_estimate = 0.0  # Initial state estimate
...
def main():
	...
	# Kalman filter parameters (adjust as needed)
	process_variance = 1e-5  # Process noise variance
	measurement_variance = 1e-1  # Measurement noise variance
	estimated_measurement_variance = 1e-1  # Estimation error

	# Create Kalman filter instance
	kf = KalmanFilter(process_variance, measurement_variance, estimated_measurement_variance)

	# Read accelerometer data and apply Kalman filter
	while True:
		# Read accelerometer data (e.g., X-axis)
		sensor.icm20948_Gyro_Accel_Read()
		sensor.icm20948MagRead()
		sensor.icm20948CalAvgValue()
		time.sleep(0.1)
		sensor.imuAHRSupdate(MotionVal[0] * 0.0175, MotionVal[1] * 0.0175,MotionVal[2] * 0.0175,
							MotionVal[3],MotionVal[4],MotionVal[5], 
							MotionVal[6], MotionVal[7], MotionVal[8])
		ax = Accel[0]
		#ax, ay, az = sensor.getAccelerometerRaw()

		# Apply Kalman filter to accelerometer X-axis data (or gyroscope data if needed)
		filtered_ax = kf.update(ax)

		# Print the raw and filtered accelerometer data
		print(f"Raw Accelerometer X: {ax:.3f} | Filtered X: {filtered_ax:.3f}")
		
		# Optional: Display raw and filtered data for other axes
		# print(f"Raw Accelerometer Y: {ay:.3f} | Filtered Y: {kf.update(ay):.3f}")
		# print(f"Raw Accelerometer Z: {az:.3f} | Filtered Z: {kf.update(az):.3f}")

		# Sleep for a while to simulate sensor update rate
		time.sleep(0.1)
